...
def create_comparison_table(test_cases: dict):
    ...
    for case_name, case_data in test_cases.items():
        ...
        dt_pct = (dt_max * (fsw_khz * 1000.0)) * 50.0 if fsw_khz else 0.0  # mantendo sua fórmula

        rows[case_name] = [
            f"{po:.1f} W",
            f"{results.get('voutAVG', 0.0):.4f}",               # <<< NOVO: VoutAVG
            f"{fsw_khz:.4f} kHz",
            f"{dt_max*1e9:.4f} ns",
            f"{dt_pct:.4f}%",
...
def get_case_data(params, vtarget, SimCycles, TimeStep, tol, plot=False):
    ...
    result, fsw_khz = find_vtarget(params=params, Vtarget=vtarget, tol=tol,
                                  SimCycles=SimCycles, TimeStep=TimeStep, Full_Arr=plot, stable_blocks_required=2, avg_cycles = 5, cycles_per_block=20,steady_state_tol=10e-3)
...
def process_case(case_name, base_params, SimCycles, TimeStep, tol, plot):
    ...
    # This function contains the minimal timing logic.
    try:
...
def test_edge_cases(desired_cases_identifiers, base_params,
                    ...
                    return_arrays=False): 
    """
    Simulates a list of LLC operating points and returns a summary.

    Each identifier in *desired_cases_identifiers* must be in the format
    "<Vbus>V/<Vout>V@<Iout>A" (e.g., "400V/70V@1.5A").

    Parameters
    ----------
    ... (other parameters) ...
    return_arrays : bool
        If True, the full waveform arrays (time, vout, etc.) are included
        in the final returned results, without generating plots.
    """
    ...
    # Determine if the simulation should generate the full arrays
    generate_full_arrays = plot or return_arrays

    test_cases = {}
    with ProcessPoolExecutor(max_workers=max_workers) as pool:
        # Pass the combined logic down to the worker function
        futs = {pool.submit(process_case, c, base_params=base_params,
                            SimCycles=SimCycles, TimeStep=TimeStep, tol=tol, plot=generate_full_arrays): c
    ...
    if plot:
        if vos:
            ...
            for cname, data in vos.items():
                plot_detailed_case_waveforms(cname, data,
                                             test_cases[cname]["fsw_khz"],
                                             base_params, num_cycles_zoom=5,
                                             output_dir=out_dir)
        else:
    ...
    if save_csv:
        if not df.empty:
            ...
            print("CSV saving was requested, but no data is available to save.")

    return [{"case": n, "fsw_khz": d["fsw_khz"], **d["results"]}
            for n, d in test_cases.items()]