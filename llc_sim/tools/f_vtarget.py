...
def simulate_until_steady_state(
    ...
    # ---- helpers (local; no change to signature) --------------------
    ...
    current_cycle = 0
    previous_avg = None
    prev_end_value = None          # for Poincaré
    final_analysis = None
    stable_block_counter = 0

    # optional node .ic handoff (voltages)
    initial_conditions = {}
    ...
    while current_cycle < MaxCycles:
        ...
        # 1) node voltages as .ic (optional, helps DC init)
...
def voltage_error(fsw_khz, params, Vtarget, tol, SimCycles, TimeStep, Full_Arr, 
    ...
    try:
        circuit = build_llc_circuit(current_params, Vtarget)
        ...
        if analysis_fast is None:
            print(f"  > Simulation failed for fsw = {fsw_khz:.3f} kHz")
            return 1e6
        ...
        print(f"[ERROR] Unexpected failure at fsw = {fsw_khz:.3f} kHz. Details below:")
        traceback.print_exc()
        return 1e6
...
def find_vtarget(params, Vtarget, tol, SimCycles, TimeStep, 
    ...
    Final simulation length is chosen based on the cycles needed to stabilize.
    """
    ...
    try:
        ...
        # --- Dynamic final length based on stabilization time ---
        try:
            current_params = params.copy()
            current_params["fsw"] = fsw_khz * 1e3  # Hz

            circuit = build_llc_circuit(current_params, Vtarget)
            stable_block, cycles_to_stable = simulate_until_steady_state(
                circuit=circuit,
                fsw=current_params["fsw"],
                TimeStep=TimeStep,
                MaxCycles=20e3,
            ...
            if stable_block is None or cycles_to_stable is None:
                final_sim_cycles = max(cycles_per_block, 5*avg_cycles)
            else:
                # Choose final length proportional to stabilization time
                final_sim_cycles = max(
                    cycles_per_block,
                    int(cycles_to_stable*(1))
                )

            print(cycles_to_stable)
            # Run a final short simulate with that dynamic length
        ...
        try:
            ...
            final_results = {
                'voutAVG': e.vout,
                'note': 'early stop; final dynamic-length sim failed or skipped'
            }
